...
namespace iOS.Fujitsu
{
	...
	[BaseType(typeof(NSObject))]
	[Protocol]
	public partial interface FTP2176000N0_IOSLibrary
	{
		[Export ("version")]
		string version { get; }

		//		- (int)startPrinter:(NSString *)model protocol:(NSString *)protocol series:(int)series;
		//[Export("startPrinter:protocol:series:")]
		[Export("startPrinter:protocolName:series:")]
		int StartPrinter(NSString model, NSString protocol, int series);
		...
		[Export("setCharacterStyle:kanjiSize:fontHSize:fontVSize:bold:bwReverse:underLine:space:leftOfKanji:rightOfKanji:")]
		int SetCharacterStyle(int size, int kanjiSize, int fontHSize, int fontVSize, bool bold, bool blackWhiteReverse, int underLine, int space, int leftOfKanji, int rightOfKanji);
		...
		[Export("setPaperType:feedBeforePrintPage:feedAfterMarkDetection:autoFeedlength:")]
		int SetPaperType(int paperType, int feedBeforePrintPage, int feedAfterMarkDetection, int autoFeedLength);
		...
		[Export("setPrintQuality:energy:halfPitch:thermalHeadDivision:")]
		int SetPrintQuality(int paperType, int energy, bool halfPitch, int division);
